...
for f in required_files:
    if not os.path.exists(os.path.join(MODEL_DIR, f)):
        raise RuntimeError(f"❌ Missing ECAPA model file: {f}")

# Load local SpeechBrain model
...
def get_ecapa_embedding_from_file(path):
    ...
    if emb.shape[0] != 192:
        raise RuntimeError(f"❌ Incorrect embedding size: {emb.shape}")
